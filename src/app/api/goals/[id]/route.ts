...
import {
  ...
  deleteGoal,
  updateGoal,
} from "@/server/services/goals/mutate";
import { updateGoalSchema } from "@/server/validators/goals";
...
// PATCH /api/goals/:id — edita nome, ícone, valores, aporte ou prazo.
export async function PATCH(
  ...
  try {
    const updated = await updateGoal(getDb(), auth.userId, id, parsed.data);
...
// DELETE /api/goals/:id — soft delete (preserva auditoria).
export async function DELETE(
  ...
  try {
    const deleted = await deleteGoal(getDb(), auth.userId, id);
    return jsonOk(deleted);
  } catch (error) {
    if (error instanceof GoalNotFoundError) {
      return jsonError(error.code, error.message, 404);
    }
    return jsonError("INTERNAL_ERROR", "Não foi possível excluir a meta.", 500);
  }
}
